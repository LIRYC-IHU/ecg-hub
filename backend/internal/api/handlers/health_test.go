...
func TestHealthHandler_Healthy(t *testing.T) {
	...
	handler := HealthHandler(&mockPinger{err: nil}, DICOMStatus{}, FTPStatus{}, ECTPStatus{})
...
func TestHealthHandler_Degraded(t *testing.T) {
	...
	handler := HealthHandler(&mockPinger{err: fmt.Errorf("connection refused")}, DICOMStatus{}, FTPStatus{}, ECTPStatus{})
	if err := handler(c); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestHealthHandler_DICOMFields(t *testing.T) {
	...
	handler := HealthHandler(&mockPinger{err: nil}, DICOMStatus{Enabled: true, Port: 11112}, FTPStatus{}, ECTPStatus{})
	if err := handler(c); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	body := rec.Body.String()
...
func TestHealthHandler_DICOMDisabled(t *testing.T) {
	...
	handler := HealthHandler(&mockPinger{err: nil}, DICOMStatus{Enabled: false, Port: 0}, FTPStatus{}, ECTPStatus{})
	if err := handler(c); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	body := rec.Body.String()
...
func TestHealthHandler_FTPFields(t *testing.T) {
	...
	handler := HealthHandler(&mockPinger{err: nil}, DICOMStatus{}, FTPStatus{Enabled: true, Port: 2121}, ECTPStatus{})
	if err := handler(c); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	body := rec.Body.String()
...
func TestHealthHandler_FTPDisabled(t *testing.T) {
	...
	handler := HealthHandler(&mockPinger{err: nil}, DICOMStatus{}, FTPStatus{Enabled: false, Port: 0}, ECTPStatus{})
	if err := handler(c); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	body := rec.Body.String()
