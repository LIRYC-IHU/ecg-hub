...
type ActionKey = 'view' | 'download' | 'hl7_force' | 'delete' | 'quarantine_decision'

const actionConfig: Record<ActionKey, { icon: React.ElementType; label: string; cls: string }> = {
...
export function AdminAuditPage() {
  ...
  function exportCSV() {
    ...
    const rows = logs.map((l) =>
      [new Date(l.created_at).toISOString(), l.user_id, l.action, l.resource_id].join(','),
    )
    ...
    <div className="p-6">
      ...
      <div className="flex gap-3 flex-wrap items-center mb-4">
        ...
        <div className="ml-auto flex items-center gap-3">
          ...
          {!isLoading && (
            <span className="text-xs text-muted-foreground">{total} {t('admin.audit.entries')}</span>
          )}
          <button
            onClick={exportCSV}
      ...
      <div className="bg-card rounded-lg border border-border overflow-hidden">
        {/* Header row */}
        <div className="grid grid-cols-[140px_1fr_110px_120px_80px] gap-4 px-4 py-2.5 bg-muted/50 border-b border-border">
          <span className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">{t('admin.audit.colDate')}</span>
          <span className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">{t('admin.audit.colUser')}</span>
          <span className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">{t('admin.audit.colAction')}</span>
          <span className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">{t('admin.audit.colResource')}</span>
          <span />
        </div>
          ...
          return (
            <div key={log.id} className="border-b border-border last:border-0">
              <div className="grid grid-cols-[140px_1fr_110px_120px_80px] gap-4 px-4 py-3 items-center hover:bg-muted/20 transition-colors">
                ...
                {/* User */}
                <span className="text-xs font-mono text-foreground truncate">{log.user_id}</span>

                {/* Action badge */}
                <span className={`inline-flex items-center gap-1 text-[11px] font-medium px-2 py-0.5 rounded-full w-fit ${cfg?.cls ?? 'bg-muted text-muted-foreground'}`}>
                  {Icon && <Icon size={10} />}
                  {log.action}
                </span>
                ...
                <div className="flex items-center gap-1.5">
                  <span className="text-xs font-mono text-muted-foreground truncate">{log.resource_id}</span>
                  {log.resource_id && (
                    <button
                      onClick={() => handleCopy(log.id, log.resource_id)}
