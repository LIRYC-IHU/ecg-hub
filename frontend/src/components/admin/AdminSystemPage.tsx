...
export function AdminSystemPage() {
  ...
  const testMutation = useMutation({
    ...
    onSuccess: (data) => {
      if (data.success)
        notify("success", `Webhook OK — HTTP ${data.status_code}`);
      else notify("warn", `Webhook — réponse HTTP ${data.status_code ?? "?"}`);
    },
    onError: () =>
      notify("error", "Webhook — impossible de joindre le serveur distant"),
  });
  ...
  const services = [
    ...
    },
    {
      ...
      icon: HardDrive,
      ok: ftpEnabled,
      label: ftpEnabled ? `:${ftpPort}` : "Désactivé",
    },
    {
      name: "Serveur DICOM",
      icon: Radio,
      ok: dicomEnabled,
      label: dicomEnabled ? `:${dicomPort}` : "Désactivé",
    },
    {
      name: "Serveur ECTP",
      icon: Activity,
      ok: ectpEnabled,
      label: ectpEnabled ? `:${ectpPort}` : "Désactivé",
    },
  ];
    ...
    <div className="p-6">
      <div className="mb-6">
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Surveillance des services et statistiques
        </p>
      </div>
        ...
        <div className="col-span-2 space-y-6">
          ...
          <div className="bg-card rounded-lg border border-border p-5">
            <div className="flex items-center gap-3 mb-4">
              <div
                className={`w-8 h-8 rounded-full flex items-center justify-center ${isOperational ? "bg-success/10" : "bg-destructive/10"}`}
              >
                {isOperational ? (
                  ...
                  <div className="space-y-1">
                    {services.map((svc) => (
                      ...
                      >
                        ...
                        <div
                          className={`w-2 h-2 rounded-full ${svc.ok ? "bg-success" : "bg-muted-foreground"}`}
                        />
                        <span
                          className={`text-[11px] font-medium ${svc.ok ? "text-success" : "text-muted-foreground"}`}
                        >
                          {svc.ok
                            ? "OK"
                            : svc.label === "Désactivé"
                      ...
                      <div className="space-y-1">
                        {modulesQuery.data.map((m) => (
                          ...
                          >
                            ...
                            <div
                              className={`w-2 h-2 rounded-full ${m.status === "ok" ? "bg-success" : "bg-warning"}`}
                            />
                            <span
                              className={`text-[11px] font-medium ${m.status === "ok" ? "text-success" : "text-warning"}`}
                            >
                              {m.status === "ok" ? "OK" : "ERR"}
                            </span>
                          </div>
                      ...
                      {connectorsQuery.data.map((conn) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {!ok && (
                              <span
                                className="text-[10px] font-mono text-destructive truncate max-w-[180px]"
                                title={conn.status}
                              >
                                {conn.status}
                              </span>
                            ...
                            />
                            <span
                              className={`text-[11px] font-medium ${ok ? "text-success" : "text-destructive"}`}
                            >
                              {ok ? "OK" : "KO"}
                            </span>
                          </div>
